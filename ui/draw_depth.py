...
def get_coord(st=(-1, -1), interval=(2, 2)):
    ...
    int_x = interval[0]
    int_y = interval[1]
    return ((x, y), (x + int_x, y), (x + int_x, y + int_y), (x, y + int_y))
...
def reset_post_pixel_matrix(context):
    ...
    view_matrix = Matrix([[w, 0, 0, -1], [0, h, 0, -1], [0, 0, 1, 0], [0, 0, 0, 1]])
    gpu.matrix.reset()
...
def draw_shader_old(width, height):
    ...
    depth_buffer = gpu.state.active_framebuffer_get().read_depth(
        0,
        0,
        width,
        height,
...
class DrawDepth(PublicClass):
    ...
    @classmethod
    def register(cls):
        cls._draw_depth_handel = bpy.types.SpaceView3D.draw_handler_add(cls.draw_depth, (), "WINDOW", "POST_PIXEL")
    ...
    def draw_gpu_buffer_new_buffer_funcs(cls, sam_width, sam_height, width, height, sampling):
        ...
        if (width, height) not in cls.depth_buffer:
            ...
            sam_all_depth = np.empty(shape=(sam_height, sam_width, 3), dtype=np.float32)
            pos = ((0, -1), (0, 0), (1, 0), (1, -1))
            ...
            batch = batch_for_shader(
                shader,
                "TRI_FAN",
                {"pos": pos, "texCoord": tex_coord},
            )
            cls.depth_buffer[(width, height)] = [depth, depth_buffer, sam_all_depth, shader, batch]
        else:
            depth, depth_buffer, sam_all_depth, shader, batch = cls.depth_buffer[(width, height)]
            gpu.state.active_framebuffer_get().read_depth(0, 0, width, height, data=depth_buffer)
            sam_all_depth[:, :, 0] = sam_all_depth[:, :, 1] = sam_all_depth[:, :, 2] = sum((depth.reshape(height, width)[i::sampling, ::sampling] for i in range(sampling))) / sampling

        sam_all_depth = np.floor(sam_all_depth.ravel())

        sam_all_depth_buffer = gpu.types.Buffer("FLOAT", sam_width * sam_height * 3, sam_all_depth)
        depth_gpu_texture = gpu.types.GPUTexture((sam_width, sam_height), format="RGB16F", data=sam_all_depth_buffer)

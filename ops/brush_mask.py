...
class MaskProperty(PublicOperator, PublicDraw):
    ...
    def is_bbrush_brush(self):  # 是bbrush的笔刷
        return self.is_polygon_mode or self.is_circular_mode or self.is_ellipse_mask_brush
    ...
    def paint_mask_box_gesture_mask_is_use_front_faces_only(self):
        return self.active_tool.operator_properties("paint.mask_box_gesture").use_front_faces_only
...
class MaskClick(MaskProperty):
    ...
    def mask_click_ctrl_alt(in_model, sculpt):
        if in_model:  # 硬化遮罩
            sculpt.mask_filter("EXEC_DEFAULT", True, filter_type="SHARPEN", auto_iteration_count=True)
        else:  # 切换遮罩
            sculpt.face_set_change_visibility("EXEC_DEFAULT", True, mode="TOGGLE")
...
class MaskDrawArea(MaskClick):
    ...
    def ellipse_data(self):
        ...
        draw_data = list((np.add(i, self.start_mouse)) for i in circular)
        return draw_data
    ...
    def draw_lift_up_text(self):
        ...
        self.draw_text(min(x1, x2), max(y1, y2), text=text, font_id=1, size=15, color=(1, 1, 1, 1))
    ...
    def draw_box(self):
        ...
        batch = batch_for_shader(shader, "TRIS", {"pos": vertices}, indices=self.indices)
        shader.bind()
        ...
        x = int((x1 + x2) / 2)
        y = int((y1 + y2) / 2)
        self.draw_line(((x - line_length, y), (x + line_length, y)), (1, 1, 1, 1), line_width=1)
        self.draw_line(((x, y - line_length), (x, y + line_length)), (1, 1, 1, 1), line_width=1)
...
class MaskClickDrag(MaskDrawArea):
    def mask_ops_lasso_path(self, path_, value):
        path = [{"name": "", "loc": i, "time": 0} for i in path_]
        bpy.ops.paint.mask_lasso_gesture("EXEC_DEFAULT", True, path=path, value=value, use_front_faces_only=self.is_use_front_faces_only)

    def exit_modal_handle(self):
        x1, y1 = self.start_mouse
        x2, y2 = self.mouse_co
        w, h = self.start_mouse - self.mouse_co
        in_modal = self.get_area_ray_cast(min(x1, x2), min(y1, y2), abs(w), abs(h))
        args = {
            "xmin": int(min(x1, x2)),
            "xmax": int(max(x1, x2)),
            "ymin": int(min(y1, y2)),
    ...
    def exit_exception(self):
        paint = bpy.ops.paint
        if self.only_ctrl:
            paint.mask_flood_fill("EXEC_DEFAULT", True, mode="VALUE", value=0)
        elif self.ctrl_alt:

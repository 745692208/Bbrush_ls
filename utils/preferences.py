...
class BBrushAddonPreferences(AddonPreferences, PublicClass):
    ...
    depth_display_mode: EnumProperty(name="剪影图显示模式", default="ONLY_SCULPT", items=depth_display_items)
    depth_scale: FloatProperty(name="剪影图缩放", default=0.3, max=2, min=0.1, step=0.1)
    depth_offset_x: IntProperty(name="剪影图偏移X", default=0, max=114514, min=0)
    depth_offset_y: IntProperty(name="剪影图偏移Y", default=80, max=114514, min=0)

    always_use_sculpt_mode: BoolProperty(name="始终使用Bbrush雕刻模式", description="如果进入雕刻模式则自动开启Bbrush模式,退出雕刻模式则退出Bbrush模式", default=False)

    depth_ray_size: IntProperty(name="深度射线检查大小(px)", description="检查鼠标是否放在模型上,鼠标范围大小", default=100, min=10, max=300)

    show_shortcut_keys: BoolProperty(name="显示快捷键", default=True)
    shortcut_offset_x: IntProperty(name="快捷键偏移X", default=150, max=114514, min=0)
    shortcut_offset_y: IntProperty(name="快捷键偏移Y", default=20, max=114514, min=0)
